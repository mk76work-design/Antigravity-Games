	...
	for y in board.height:
		for x in board.width:
			...
			if board.is_wall(pos):
				...
				_add_static_box(_grid_to_world(pos, FLOOR_HEIGHT * 0.5), Vector3(CELL_SIZE, FLOOR_HEIGHT, CELL_SIZE), COLOR_FLOOR)
...
func sync(board: CargoBoard) -> void:
	...
	if moved_from != Vector2i(-1, -1) and moved_to != Vector2i(-1, -1):
		...
		_tween_to(mesh_instance, _grid_to_world(moved_to, FLOOR_HEIGHT + BOX_SIZE * 0.5))
		_set_box_color(mesh_instance, COLOR_BOX_ON_TARGET if board.is_target(moved_to) else COLOR_BOX)
